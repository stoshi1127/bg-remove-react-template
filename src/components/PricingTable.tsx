    ...
    <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-stretch ${className}`} role="region" aria-label="FreeとProの比較">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8 flex flex-col h-full">
        <div className="mb-6 md:mb-8">
          ...
          <p className="text-slate-500 text-sm">まずは登録不要で基本機能から試したい方へ</p>
        ...
        <div className="flex-grow space-y-3 md:space-y-4 mb-6 md:mb-8">
          {features.map((feature) => (
            <div key={`free-${feature.label}`} className="flex items-center justify-between py-2 border-b border-slate-100">
              ...
              <span className={feature.free === '利用不可' || feature.free === '-' ? 'text-slate-400 text-sm md:text-base' : 'font-medium text-slate-800 text-sm md:text-base'}>{feature.free}</span>
            </div>
          ))}
        </div>
      ...
      <div className="relative bg-white rounded-xl shadow-xl border-2 border-pro-orange p-6 md:p-8 flex flex-col h-full transform md:scale-[1.02] transition-transform">
        ...
        <div className="mb-6 md:mb-8">
          <h2 className="text-xl font-bold mb-2 flex items-center gap-2 text-gray-900 justify-center">
            Proプラン
            <span className="material-symbols-outlined text-pro-orange" style={{ fontVariationSettings: "'FILL' 1" }}>verified</span>
          </h2>
          <p className="text-slate-500 text-sm">全ての機能で最高のクリエイティブを</p>
          <div className="mt-4 md:mt-6">
        ...
        <div className="flex-grow space-y-3 md:space-y-4 mb-6 md:mb-8">
          {features.map((feature) => (
            <div key={`pro-${feature.label}`} className="flex items-center justify-between py-2 border-b border-orange-50">
              <span className="text-slate-600 flex items-center gap-2 font-medium text-sm md:text-base">
              ...
              <div className="flex flex-col items-end">
                <span className="font-bold text-gray-800 text-sm md:text-base">{feature.pro}</span>
