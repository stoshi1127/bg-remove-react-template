        ...
        <div id="premium-features" className="mt-16 md:mt-24 w-full scroll-mt-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                {FEATURES.map((feature, index) => (
                    <div
                        key={index}
                        className={`bg-white/50 p-3 md:p-4 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden ${feature.isComingSoon ? 'opacity-50' : ''}`}
                    >
                        {feature.isComingSoon && (
                            <div className="absolute top-3 right-[-35px] bg-slate-200 text-slate-600 text-[10px] font-bold py-1 px-10 rotate-45 shadow-sm">
                                準備中
                            </div>
                        )}
